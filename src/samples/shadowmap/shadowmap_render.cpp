...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  m_ColorMap = m_context->createImage({
...
void SimpleShadowmapRender::loadShaders()
{
  ...
  etna::create_program("tone mapping", {
    VK_GRAPHICS_BASIC_ROOT"/resources/shaders/full_screen.vert.spv",
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_ToneMapping = pipelineManager.createGraphicsPipeline("tone mapping", {
    .vertexShaderInput = {
    },
    .fragmentShaderOutput = {
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    ...
    etna::RenderTargetState renderTargets(a_cmdBuff, {0, 0, m_width, m_height},
      {{.image = m_ColorMap.get(), .view = m_ColorMap.getView({})}},
  ...
  {
    auto toneMapping = etna::get_shader_program("tone mapping");

    auto set = etna::create_descriptor_set(
      toneMapping.getDescriptorLayoutId(0),
      a_cmdBuff,
      {
        { 0, constants.genBinding() },
        { 1, m_ColorMap.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal) }
      }
    );
    ...
    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_ToneMapping.getVkPipeline());

    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_ToneMapping.getVkPipelineLayout(), 0, std::size(sets), sets, 0, VK_NULL_HANDLE);
