...
void main()
{
  ...
  {
    const vec2 depthMoments = textureLod(shadowMap, shadowTexCoord, 0).xy;
    ...
    {
      const float depthMean = depthMoments.x;
      const float depthVariance = depthMoments.y - depthMoments.x * depthMoments.x;

      shadow = depthVariance / (depthVariance + (depth - depthMean) * (depth - depthMean));
    }
  }
