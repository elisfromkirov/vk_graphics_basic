...
void main()
{
  ...
  float angle = acos(dot(normalize(surf.wPos - Params.lightPos), normalize(Params.lightDir)));
  float intensity = 0;
  if (angle < HALF_PI)
  {
    intensity = 1.0 - clamp((angle - Params.lightInnerAngle) / (Params.lightOuterAngle - Params.lightInnerAngle), 0.0, 1.0);
  }
  vec4 lightColor = intensity * lightColor1;
